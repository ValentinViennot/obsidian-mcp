...
MARKER = "embedding validity interval (026_temporal_embeddings)"
...
def _reconcile_column(bind, column: str) -> None:
    """Create the column, or verify a pre-existing one is exactly 026's.
    ...
    if notnull:
        problems.append("it is NOT NULL; 026 creates it nullable")
    if default is not None:
        problems.append(f"it carries a server default ({default!r}); 026 sets none")
    if comment != MARKER:
        problems.append("it does not carry 026's comment marker")
    if problems:
        raise RuntimeError(
            f"{TABLE}.{column} already exists but {'; '.join(problems)}. 026 "
            ...
            "Resolve by hand — drop it and let 026 create it, or make it match "
...
def _reconcile_index(bind) -> None:
    ...
    026's marker — an index of the same name over different columns would be
    ...
    if comment != MARKER:
        raise RuntimeError(
            ...
            f"not carry 026's comment marker ({MARKER!r}), so 026 did not "
...
def downgrade() -> None:
    """Delete the historical rows, then drop 026's index and columns.
    ...
    **The row deletion is not optional and its order is not arbitrary.** A
    ...
    for column, state in present.items():
        if state[3] != MARKER:
            raise RuntimeError(
                f"{TABLE}.{column} does not carry 026's comment marker "
                f"({MARKER!r}), so 026 did not create it and will not drop it. "
    ...
    if exists:
        if comment != MARKER:
            raise RuntimeError(
                ...
                f"carry 026's comment marker ({MARKER!r}), so 026 did not "
