...
| `panel_oidc_login_refused` | WARNING | `client_ip`, `reason`, `route`, `user_id` | `src/auth/routes.py` `oidc_callback` and `_redirect_to_provider` (`AUTH_MODE=pocketid`), every refusal branch. `reason` is a closed vocabulary and every value is this server's own classification, never provider- or caller-authored text: `no_login_in_flight` / `state_mismatch` / `provider_refused` / `no_code` / `group_required` / `no_username_claim` / `username_claimed` / `inactive_user` / `session_mint_refused`, plus the ones `src/auth/oidc.py` raises — `provider_unreachable` / `provider_error` / `insecure_endpoint` / `discovery_incomplete` / `issuer_mismatch` / `jwks_invalid` / `unknown_key` / `unsupported_algorithm` / `bad_signature` / `audience_mismatch` / `token_expired` / `token_not_yet_valid` / `token_invalid` / `nonce_mismatch` / `token_exchange_failed`. The rendered page is identical across all of them, so this is the only place the cause exists. **Never** the ID token, the authorization code, `state`, `nonce`, the PKCE verifier or the `sub` claim — no field in the allow-list can carry any of them |
| `panel_oidc_user_created` | INFO | `client_ip`, `user_id`, `username` | `src/auth/routes.py` `oidc_callback`, after the commit — a provider identity that had no local account. The row is created `is_admin=False` with no `vault_path`; nothing is ever auto-promoted |
| `panel_oidc_user_linked` | INFO | `client_ip`, `user_id`, `username` | `src/auth/routes.py` `oidc_callback`, after the commit — an **existing** local account (with whatever vault, keys and admin flag it already had) bound to a provider identity on that identity's first login. Distinct from the creation above because an operator audits the two differently |
