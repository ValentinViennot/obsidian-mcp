...
- `delete_note(path, permanent=False, expected_hash=None)` — soft-delete to `.trash/<YYYYMMDD-HHMMSS>-<basename>-<8 hex>` by default (the same non-replacing `renameat2` `delete_file` uses; the hex suffix is what makes two same-second deletes distinct); `permanent=True` unlinks through the parent descriptor. The indexer skips dot-dirs, so search/embedding cleanup happens on the next reindex pass.
- `set_frontmatter(path, updates, remove=[], expected_hash=None)` — structured YAML frontmatter mutation. Round-trips via `yaml.safe_dump` (does not preserve YAML comments). Leaves the body byte-identical. **Refuses a malformed block by name** rather than prepending a second one; only an effective mutation writes.
...
- a **read-modify-write overwrite** (`edit_note`, `set_frontmatter`, and `move_note`'s link rewrites) is optimistic, not linearizable: `expected=` compares the current bytes immediately before the rename and a writer landing inside that window is still overwritten — the same guarantee level as the transfer fingerprint check, declared rather than implied. **`expected=` is server-internal and its window is the tool's own read→rename interval, not the caller's.** The bytes it compares are the ones this call read, so it structurally cannot see a change that landed between the *caller's* `read_note` and its write. That second window is what the caller-visible **`expected_hash` precondition** closes (#205) — see "Write preconditions" below. The two are a **pair**: neither subsumes the other, neither is removed, and a matching `expected_hash` does **not** exempt a call from the `expected=` compare. Any wording that lets an agent read `File changed while editing` as covering its own read→edit round trip is still a bug in the docstring; so is any wording that lets it read `expected_hash` as covering the publish.
- **An unguarded `write_file(overwrite=True)` remains an unconditional replace** when required mode is off. Supplying `expected_hash` adds the caller-to-call check and passes the observed bytes to the same final comparison the note tools use. `request_upload(overwrite=True)` separately binds the incumbent's fingerprint at mint time.
...
## Write preconditions: `expected_hash` and `content_hash` (#205)

The failure this closes is **lost update from stale client state**: an agent reads a note, spends a turn deciding, and writes back a result computed from bytes that are no longer on disk. `expected=` cannot see it — it compares the bytes *this call* read. So the caller gets a token for the bytes *it* read.

**The digest, defined once.** `content_hash` is `"sha256:"` + the **lowercase** hex SHA-256 of the **complete raw bytes of the file as stored on disk** — every byte, in file order, no universal-newline translation, no frontmatter stripping, no windowing, no re-encoding. 71 characters. `vault.content_hash_for_bytes` is the only producer; the formal requirement lives in `openspec/specs/vault-write/`. Three things it is not, and every docstring that mentions it says so:

- **not a hash of any body or response field.** A note with frontmatter, or with CRLF terminators, has a `content_hash` that a hash of `read_note`'s `content` cannot reproduce. It is a token to hand back, never something a caller computes.
- **not `notes_metadata.content_hash`.** Same algorithm, different input — that column hashes the *translated text*, deliberately, so a CRLF note's hash does not churn. Do not redefine it; doing so would mark every CRLF note changed and re-embed the vault.
- **not section-scoped.** `#N` ordinals are positional, so a body-only digest could certify an unchanged body while an insertion above it changed which section the selector names. Whole-file only, in every read mode and every write mode — which makes the narrowest write mode the most conflict-prone, and is why the argument is optional.

**Only the canonical form is accepted on input.** `sha256:<64 lowercase hex>` and nothing else. Bare hex, uppercase hex, surrounding whitespace, another algorithm prefix, a wrong length — each is `malformed_precondition`, never a mismatch. That is what turns "you handed me the index column's value" into a message the caller can act on rather than a conflict it cannot diagnose.

### Two windows, and they are a pair

| window | closed by | refusal |
| --- | --- | --- |
| the caller's read → **this call's read** | `expected_hash` (caller-supplied, optional) | `stale_precondition`, naming the file's current hash |
| this call's read → **this call's rename** | `expected=` (server-internal, always on where it applies) | `concurrent_write`, prose unchanged |

Neither subsumes the other and neither was removed. A matching `expected_hash` does **not** disable the `expected=` compare; a test asserts an agent can tell the two apart **by code** rather than by prose shape. What remains after both is the rename syscall itself, against an adversary who can already write the destination directory — the residual declared since #59.

### The precedence ladder, and why the order is the design

1. **syntax** — a non-canonical hash is `malformed_precondition`, decided **at the tool's entry: before path resolution, before `_leaf_state_error`, before any read**, in every tool and every mode. `_precondition_syntax_error` is therefore a *separate* helper from the comparison, and a pure function of the argument. It wins over not-found, over a symlinked leaf and over the size cap — a caller told "not found" for a call whose argument was never valid fixes the wrong thing.
2. **no incumbent** — a tool or mode that can never bind incumbent bytes (`create_note`, `write_file(overwrite=False)`, an overwrite of a path with no file at it) is `no_incumbent`.
3. **unavailable** — an incumbent larger than the cap this tool may read is `precondition_unavailable`, naming the cap. "I could not check" and "I checked and it differs" are different facts; answering the first with the second sends the caller after a hash it can never obtain.
4. **required** — an enforceable call supplying no hash while `WRITE_PRECONDITION_REQUIRED` is on is `precondition_required`. Over-cap **outranks** it, for the same reason.
5. **comparison** — a digest that differs is `stale_precondition`.
6. **publication** — the existing in-call compare, reported as `concurrent_write`.

**The comparison runs immediately after the in-call read and before mode dispatch, the result size cap, the `dry_run` diff and every no-op or defect determination.** Ordering here is observable and therefore normative: a unified diff, a "no changes" answer, or a frontmatter-defect report computed against a base the caller does not hold is a *wrong answer*, not a cheap one. Do not "optimise" the check later in the function.
...
**The final byte comparison is bounded too.** `_atomic_write_at` reads at
most `len(expected) + 1` bytes through the pinned parent. An `fstat` size over
`len(expected)` refuses before reading; growth after that stat is caught by
the extra byte. Either means `concurrent_write`, because a longer file cannot
match the expected bytes. `_ReadLimitExceeded` remains a `ValueError` for
ordinary bounded-read callers but lets publication distinguish growth from a
leaf that became a non-regular file, preserving that existing error behavior.
The bound applies to both note and raw-file publication, including an empty
incumbent, and leaves the final compare-to-rename race unchanged.
...
| `set_frontmatter` | optional | whole file | ✅ the bytes published (a no-op reports none) |
| `create_note` | ⛔ `no_incumbent` | — | ✅ the bytes published |
| `write_file(overwrite=True)`, path present | optional | whole file | ✅ the bytes published |
| `write_file(overwrite=True)`, path absent | ⛔ `no_incumbent` when supplied | — | ✅ when unguarded and it creates |
| `write_file(overwrite=False)` | ⛔ `no_incumbent` | — | ✅ the bytes published |
| `move_note` | optional | the **source note's** bytes | ✅ the destination's — see the matrix |
...
| `import_from_url(overwrite=True)`, `request_upload(overwrite=True)` | ⛔ out of scope | — | — |

"⛔" is load-bearing: **the argument is accepted by the signature on every one of those tools** and answered with a typed refusal. A signature that rejected it would produce a protocol-level argument error instead of the contract — which is exactly why `create_note` takes the argument it can never honour, and why `create_note` evaluates it *before any filesystem work*.

**The reported hash is always the bytes the call actually published**, never the bytes it intended to publish. An intended-but-unpublished hash would hand the agent a token that binds nothing and make its next guarded write fail against bytes that were never written.

### `move_note`'s post-rename partial-success contract

A `rewrite_links=True` move publishes **twice** — the `renameat2`, then the moved note's own body rewrite — and the second can fail after the first has committed.

| case | reported `content_hash` |
| --- | --- |
| plain move (`rewrite_links=False`) | the moved bytes, unchanged from the source |
| the moved note's own rewrite **published** | the post-rewrite bytes at `to_path` |
| that rewrite **failed without observing a change** (I/O error, a stopped loop under a reassignment or a confirmation outage) | the **rename's** bytes — what is actually at `to_path` |
| that rewrite **lost the in-call conflict** (`concurrent_write`) | **none at all** |
| a **backlink source's** rewrite failed | unaffected: the moved note's, per the rows above |

The conflict row is the sharp one: the destination holds a third writer's bytes that this call never read, so the *rename's* hash is wrong too. The result omits the hash entirely and says the move completed, the link rewrite did not, and the destination must be re-read before it is written to.

**After a rename, nothing may claim that nothing was written.** A post-rename failure is a partial success, never a whole-call refusal, and `nothing_written: true` must not appear on it in any form — the move happened, and a caller told otherwise goes looking for a note that has already relocated. `_concurrent_write_refusal(…, nothing_written=None)` exists for exactly this. The moved note's own rewrite is identified by `write_target is dst_target`, not by a path comparison, because identity is exact and a name can repeat.
...
### The refusal codes, and the remedy each one carries

Every refusal is the shared shape from `src/services/refusals.py`: the tool's prose, then a **final, line-initial, single-line** `MCP-REFUSAL {…}` sentinel. Fields that do not apply are **absent**, never null. **No code carries a retry delay** — no interval makes a stale hash match, a malformed one canonical, or a file smaller than a cap — and each code's prose states the action that resolves *it*, because one generic "re-read and retry" would be wrong for three of the six.

| code | carries | the remedy its prose states |
| --- | --- | --- |
| `stale_precondition` | `path`, `current_hash`, `nothing_written` | re-read, recompute from the current bytes, resend; the named hash is resendable if nothing else changes |
| `concurrent_write` | `path`, `nothing_written` (**absent** after a rename) | the file changed *during* the call, so no pre-call hash can be valid: re-read and retry |
| `no_incumbent` | `path`, `nothing_written` | there is nothing to guard here — **call again without `expected_hash`** |
| `malformed_precondition` | `path`, `nothing_written` | the canonical form is `sha256:<64 lowercase hex>`, and where a valid one comes from |
| `precondition_unavailable` | `path`, `cap_name`, `cap_bytes`, `nothing_written` | the cap and its value; raising it is an operator action, and the transfer download route is the way to fetch the bytes |
| `precondition_required` | `path`, `nothing_written`, `current_hash` **only where the tool had already read the incumbent** | this deployment requires a precondition; resend with `expected_hash` |

`concurrent_write`'s prose is **unchanged** — `File changed while editing: <name>` — with the sentinel appended. Every existing `in` / `startswith` assertion still holds; that is the additive rule, and it is what lets the two windows differ by code rather than by prose shape. A refusal carries **no note content**: no excerpt, no diff, no length. `path` is bounded as every other path-bearing message is.

**`WRITE_PRECONDITION_REQUIRED`** (a `Settings` boolean, default **false**) makes every enforceable tool refuse an unguarded call. It is a **deployment** decision, never a per-call one: an agent cannot turn it off, and a client that does not send hashes is broken deliberately and visibly rather than silently exposed. `create_note` and `write_file(overwrite=False)` are exempt — requiring a hash where none can exist would make creation impossible. Under required mode `move_note` and `delete_note` *do* read the incumbent, because that read is the only way to tell an over-cap file from a missing hash and the ladder ranks those in that order; having read, their refusal carries `current_hash` too.

### Residuals, declared

| # | limitation | why it is accepted |
| --- | --- | --- |
| L1 | The precondition is optimistic, not linearizable. A writer landing between the compare and the rename is still overwritten; a delete or a move has no `expected=` analogue, so its window is compare → `renameat2`. | No syscall offers compare-and-swap on a rename. The #59 residual, narrowed rather than widened. |
| L3 | `move_note(rewrite_links=True)` binds only the moved note; the backlink sources it rewrites are unbound, and the refusal prose says so. | The caller never read them. Each source's own rewrite still goes through `expected=`, so a concurrent edit yields the existing partial-success report naming it. |
| L6 | `notes_metadata.content_hash` and the wire `content_hash` stay two different digests of one note. | Redefining the column would mark every CRLF note changed and re-embed the vault. Canonical-only input makes handing one where the other belongs a `malformed_precondition` rather than a confusing mismatch. |
| L10 | "Nothing was written" excludes the call's own `usage_logs` row, written for a refusal exactly as for any other call. `expected_hash` is in the logged parameter list of every tool that accepts it. | That row is the audit trail; suppressing it would hide precisely the calls an operator investigating a lost update needs. The guarantee is about the vault and the derived index. |
| L11 | Backward compatibility is claimed for **mutation and conflict semantics only**. Success strings changed: every publishing write now appends its `content_hash`, and `move_note`'s reports the destination's. Refusal prose is unchanged; the sentinel is appended to it. | Result strings are prose that has never been a contract, and the new field is additive at the end. Stated so nobody reads "no client is broken" as covering exact output text. |
...
| `content_hash` | none needed either: server-controlled and **fixed at 71 characters** (`sha256:` + 64 lowercase hex), so it is a constant allocation beside `path` in the arithmetic below. It is deliberately **outside** the metadata budget and **outside the drop order** — dropping the precondition token would silently disable the precondition on exactly the notes big enough to be worth guarding (#205, D6) |
...
**Since #205 the metadata budget's worst case is reachable through `tags`, not `title`.** The shared title rule bounds `title` to 512 characters on every surface (design D10b/L13), so a title can no longer fill the budget on its own; a note with thousands of tags can. Any test that used to load the budget with a giant title has to use `tags` instead — the drop order is unchanged, only which field can plausibly overflow it. `content_hash` is never a candidate: it is a fixed 71-character server-controlled allocation outside this budget entirely.
...
test file rather than left unmeasured. The n-versus-2n ratio benchmark
uses process CPU time so unrelated CI/agent scheduling does not look like
superlinear work (#244). Absolute DoS ceilings continue using wall time.
